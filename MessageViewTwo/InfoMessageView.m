...
static InfoMessageView *_AKSInfoMessageView = nil;

@implementation InfoMessageView

@synthesize imageView,messageView;

+ (InfoMessageView *)sharedAKSInfoMessageView
{
	TCSTART
    ...
    dispatch_once(&pred, ^{
		if (_AKSInfoMessageView==nil) {
			_AKSInfoMessageView = [[[NSBundle mainBundle] loadNibNamed:@"InfoMessageView"
...
-(void)finish2:(NSTimer*)Sender
{
	...
	{
		[self infoMessageFor:0 withMessage:@"" withFontSize:0 ShowTextOnly:YES isAnimationRequired:NO ClearPreviousMessages:NO ColorWithR:0 G:0 B:0 IsBlinking:NO IsWritingAnimation:NO Frame:CGRectZero];
		return;
	}
